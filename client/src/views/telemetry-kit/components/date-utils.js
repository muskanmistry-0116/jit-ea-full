// ------- Small helpers used by DatePackage and pages -------
...
// Plant “day” starts at Shift A start
...
export function singleDateShiftRange(dateYMD, shiftKey, shifts) {
  ...
  const base = new Date(y, m - 1, d, 12, 0, 0, 0); // noon local to avoid DST edges
  if (shiftKey === 'ALL') {
...
export function presetRange(kind, now, shifts) {
  ...
  switch (kind) {
    case 'currentShiftSoFar': {
      ...
      const t = N.getTime();
      const inW = (w) => t >= w.start.getTime() && t < w.end.getTime();
      const tag = inW(A) ? 'A' : inW(B) ? 'B' : 'C';
    ...
    case 'currentWeekSoFar': {
      ...
      const mondayOffset = (s0.getDay() + 6) % 7; // Monday=0
      const s = new Date(s0);
      s.setDate(s0.getDate() - mondayOffset);
