// src/views/alertLogPage/date/DatePackage.jsx
...
import { clampTo3Months, fmtDate, formatYMD, presetRange, rangeWithShift, startOfPlantDay } from './utils';
...
/** Vertical settings dialog for shifts */
/** Vertical settings dialog for shifts (now with conflict warning) */
function ShiftSettingsDialog({ open, onClose }) {
  ...
  // ---- validation (detect overlapping shift windows, including overnight) ---
  const toMin = (hhmm) => {
  ...
  // Return one or two [start,end) segments in minutes on a 0..1440 timeline
  const segmentsFrom = (start, end, key) => {
    const s = toMin(start);
    const e = toMin(end);
    if (Number.isNaN(s) || Number.isNaN(e)) return [];
    if (e <= s) {
      // overnight: e.g. 22:00 → 06:00  ->  [1320,1440) & [0,360)
      return [
        { s, e: 1440, key },
        { s: 0, e, key }
      ];
  ...
  // Build all segments and look for overlaps
  const conflicts = (() => {
    const segs = [
  ...
  const save = () => {
    if (hasConflicts) return; // guard (Save button will also be disabled)
    setShifts(local);
    onClose();
  };
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
        <Typography variant="body2" sx={{ mb: 2 }}>
          Customize your 8-hour shifts. Overnight is supported (e.g., 22:00 → 06:00).
        </Typography>
        ...
        {hasConflicts && (
          ...
          >
            ...
            <Typography variant="caption" sx={{ display: 'block', mt: 0.5, opacity: 0.85 }}>
              Adjust the hours so that A, B, and C do not overlap.
            </Typography>
          </Paper>
        )}
...
/** Main Date Package (no tabs; presets + range in one panel) */
export default function DatePackage({ start, end, onChange }) {
  ...
  // pick logic: two clicks → single day works (end === start)
  const pick = (ymd) => {
  ...
  const applyRange = () => {
    ...
    // Build the continuous bounds for the server/time bar
    const r = rangeWithShift(selStart, endYMD, shiftSel, shifts);

    // Also pass exact per-day shift window details so the fetcher
    // can enforce a UNION of daily windows (A/B/C) instead of one big window.
    const shiftCfg = shifts[shiftSel] || {};
    const meta =
      shiftSel === 'ALL'
        ? null
        : {
            ...
            startYMD: selStart,
            endYMD,
            shiftStart: shiftCfg.start,
            shiftEnd: shiftCfg.end
          };
  ...
  // run preset immediately (also clears manual selections)
  const onPresetChange = (val) => {
    ...
    onChange?.({
      ...
      label: r.label,
      // Presets ignore shift selection; show ALL for these.
  ...
  const Header = (
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1}>
        <Typography sx={{ fontWeight: 800 }}>Shift Setting</Typography>
