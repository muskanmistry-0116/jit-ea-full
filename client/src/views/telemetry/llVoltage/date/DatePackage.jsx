...
/* ---- shift settings dialog (with overlap warning) ---- */
...
/* ---- main DatePackage (presets + range + shift) ---- */
export default function DatePackage({ start, end, onChange }) {
  ...
  const pick = (ymd) => {
    ...
    if (!selStart || (selStart && selEnd)) {
      setSelStart(y);
      setSelEnd('');
    } else if (new Date(y) < new Date(selStart)) {
      setSelEnd(selStart);
      setSelStart(y);
    } else {
      setSelEnd(y || selStart);
    }
  };
  ...
  const onPresetChange = (val) => {
    ...
    onChange?.({ start: r.start.toISOString(), end: r.end.toISOString(), label: r.label, meta: { preset: val, shift: 'ALL' } });
    handleClose();
  };
