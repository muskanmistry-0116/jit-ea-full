// forms/components/PFTableField.jsx
...
const PFTableField = ({ field, value, onChange, error, externalSchemeKey }) => {
  ...
  // Push the displayed rows up so the backend receives exactly what's shown
  useEffect(() => {
                ...
                {field.columns.map((col) => (
                  <TableCell
                    ...
                    sx={{
                      ...
                      // Conditional colors
                      ...(col.id === 'incentive' && row[col.id] === '0%' && { color: 'info.main' }), // blue
                      ...(col.id === 'incentive' && row[col.id] !== '0%' && { color: 'success.main' }), // green
                      ...(col.id === 'penalty' && row[col.id] === '0%' && { color: 'info.main' }), // blue
                      ...(col.id === 'penalty' && row[col.id] !== '0%' && { color: 'error.main' }) // red
                    }}
                  >
                    {row[col.id]}
                  </TableCell>
