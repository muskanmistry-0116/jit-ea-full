// src/ui-component/historical/RTW15-H/components/LoadView.jsx
...
const LoadView = () => {
  ...
  const contentWrapperStyle = {
    ...
    flexWrap: 'wrap',
    marginTop: '20px',
    maxWidth: '1200px', // Explicit max width for the content area
    margin: '20px auto' // Centering the content
  };

  const chartContainerStyle = {
    flex: '1',
    minWidth: '400px', // Ensure the chart has enough room
    maxWidth: '500px'
  };

  const tableContainerStyle = {
    flex: '1',
    minWidth: '500px', // Ensure the table has enough room
    maxWidth: '600px'
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '10px' }}>
        ...
        <div>
          <span>Shift:</span>
          {['All', 'Shift A', 'Shift B', 'Shift C'].map((s) => (
            <button key={s} style={buttonStyle(shift === s)} onClick={() => setShift(s)}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div style={contentWrapperStyle}>
        <div style={chartContainerStyle}>
          <DonutChart data={chartData} totalHours={totalHours} title="Distribution of Load Bands" />
        </div>
