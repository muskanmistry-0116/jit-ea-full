...
function MonthGrid({ base, selStart, selEnd, hoverKey, onPick, onHover }) {
  ...
  const cells = Array.from({ length: 42 }).map((_, i) => {
    ...
    // live preview if hovering while picking end
    const preview =
      s && !selEnd && hoverKey
...
export default function UnifiedDateControls({
  ...
  const QuickChips = (
    <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}>
      {[
        ...
        <Chip key={q.k} label={q.label}
          onClick={() => { const [s, e] = q.f(); setSelStart(ymd(s)); setSelEnd(ymd(e)); }}
          size="small" sx={{ borderRadius: 999, fontWeight: 800, bgcolor: 'grey.100', '&:hover': { transform: 'translateY(-1px)' } }} />
      ))}
    </Stack>
  );

  const AggregationChips = (
    <>
      <Typography sx={{ fontWeight: 900, mt: 1, mb: 0.5 }}>Aggregation</Typography>
      <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }}>
        {['Average', 'Min', 'Max', 'Sum'].map((label) => (
          <Chip key={label} label={label} clickable onClick={() => setAgg(label)}
            color={agg === label ? 'primary' : 'default'} variant={agg === label ? 'filled' : 'outlined'}
            sx={{ borderRadius: 999, fontWeight: 900, boxShadow: agg === label ? '0 8px 18px rgba(59,130,246,0.3)' : 'none', '&:hover': { transform: 'translateY(-1px)' } }} />
        ))}
      </Stack>
    </>
  );
  ...
  const Footer = (applyAction) => (
    <Box sx={{ position: 'sticky', bottom: -8, pt: 2 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25} alignItems="center" justifyContent="space-between">
        <Paper elevation={0}
          sx={{ px: 2, py: 0.75, borderRadius: 2, background: 'linear-gradient(90deg, rgba(59,130,246,0.12), rgba(16,185,129,0.12) 50%, rgba(139,92,246,0.12))', border: '1px solid rgba(59,130,246,0.35)', fontWeight: 900 }}>
          <Typography sx={{ fontWeight: 900 }}>
            {mode === 'history' ? (
              <>
                ...
                (Shift {shiftSel}){selectedDaysCount ? ` • ${selectedDaysCount} day${selectedDaysCount > 1 ? 's' : ''}` : ''} • Agg: {agg}
              </>
            ) : (
              <>Last: {rtw} • Agg: {agg}</>
            )}
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button onClick={applyAction} variant="contained" size="small"
  ...
  return (
    <>
      ...
      >
        {/* Sticky header */}
        <Box sx={{ position: 'sticky', top: 0, pb: 1, background: 'inherit', zIndex: 1 }}>
        ...
        {mode === 'history' ? (
          <>
            ...
            <FormControl fullWidth size="small" sx={{ mb: 1, minWidth: 260 }}>
              <Select
                ...
                displayEmpty
                renderValue={(val) => val ? PRESET_LABELS[val] : PRESET_LABELS[DEFAULT_PRESET]}
              >
          ...
          <>
            ...
            {/* Aggregation (Realtime only) */}
            <Box sx={{ mb: 1 }}>{AggregationChips}</Box>

            {Footer(applyRealtime)}
          </>
        )}
      </Popover>
