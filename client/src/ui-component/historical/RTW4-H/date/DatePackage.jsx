...
/** Shift settings dialog (with overlap detection) */
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
        {hasConflicts && (
          <Paper variant="outlined" sx={{ px: 2, py: 1.2, mb: 2, borderColor: 'warning.light', bgcolor: 'warning.light' }}>
...
/** Main Date Package (presets + two-month range + shift select) */
export default function DatePackage({ start, end, onChange }) {
  ...
  const pick = (ymd) => {
    // NOTE: clampTo3Months is now a no-op; preserving the call keeps public API intact.
    ...
    if (!selStart || (selStart && selEnd)) {
      ...
      if (new Date(ymdC).getTime() < new Date(selStart).getTime()) {
        ...
        // If user clicks the same date twice, we lock to single-day (as required)
  ...
  const applyRange = () => {
    if (!selStart) return;
    const endYMD = selEnd || selStart; // single day if same
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems="center" justifyContent="space-between" sx={{ mt: 2 }}>
          <Paper elevation={0} sx={{ px: 2, py: 0.5, borderRadius: 2, bgcolor: 'primary.light', color: 'primary.dark', fontWeight: 700 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
              ...
              {selEnd ? new Date(selEnd).toLocaleDateString() : selStart ? new Date(selStart).toLocaleDateString() : '—'} &nbsp;({shiftSel})
            </Typography>
          </Paper>
