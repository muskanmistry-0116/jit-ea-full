...
export default function PhasePowerHistoryChart({ data }) {
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        if (originalDataPoint) {
          // --- CHANGE: Order matches the new visual stack (top to bottom) ---
    ...
    // --- CHANGE: Legend data is now an array of objects to force correct colors ---
    legend: {
      data: [
    ...
    // --- CHANGE: Series reordered for correct visual stacking (Red on top) ---
    series: [
      {
        name: 'B-Phase',
        type: 'line',
        ...
        lineStyle: { width: 0 },
        showSymbol: false,
        areaStyle: { color: phaseColors['B-Phase'] },
      ...
      },
      {
        ...
        lineStyle: { width: 0 },
        showSymbol: false,
        areaStyle: { color: phaseColors['Y-Phase'] },
        emphasis: { focus: 'series' },
        data: yKvaData
      },
      {
        ...
        lineStyle: { width: 0 },
        showSymbol: false,
        areaStyle: { color: phaseColors['R-Phase'] },
        emphasis: { focus: 'series' },
        data: rKvaData
      }
    ]
