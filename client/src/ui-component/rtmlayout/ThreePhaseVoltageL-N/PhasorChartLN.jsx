...
// ---- Config ----
// --- CHANGE: Adjusted for L-N Voltage Range ---
const MAX_VOLT = 300; 
// --- CHANGE: Updated phase names for L-N ---
...
// --- Define Thresholds ---
// --- CHANGE: Updated for L-N Voltage Range ---
const NOMINAL_VOLT = 200; 
const ACCEPTABLE_PCT = 0.10; // +/- 10%
const WARNING_PCT = 0.20;    // +/- 20%

// --- Logic to determine voltage status ---
export const getVoltageStatusLN = (voltage) => {
...
// --- Main Component ---
export default function PhasorChartLN(props) {
  ...
  useEffect(() => {
    ...
    const timerId = setTimeout(() => {
      animationIntervalRef.current = setInterval(() => {
        ...
        volts.forEach((v, i) => {
          const status = getVoltageStatusLN(v);
  ...
  const upperWarningLimit = NOMINAL_VOLT * (1 + WARNING_PCT);
  const upperAcceptableLimit = NOMINAL_VOLT * (1 + ACCEPTABLE_PCT);
  const lowerAcceptableLimit = NOMINAL_VOLT * (1 - ACCEPTABLE_PCT);
  const lowerWarningLimit = NOMINAL_VOLT * (1 - WARNING_PCT);
  ...
  return <ReactECharts ref={chartRef} echarts={echarts} option={phasorOption} notMerge lazyUpdate style={{ height: '100%', width: '100%' }} />;
}
