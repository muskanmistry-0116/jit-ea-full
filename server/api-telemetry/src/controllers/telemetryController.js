...
const QUESTDB_TBL = process.env.QUESTDB_TBL;
...
exports.getTelemetry = async (req, res) => {
  try {
    ...
    let sql = `SELECT * FROM ${QUESTDB_TBL} WHERE DID='${did}' ORDER BY ts DESC LIMIT ${limitStmt}`;
    ...
    switch (query.segment) {
      case "voltage":
        sql = `SELECT TS,VR,VY,VB,VRY,VYB,VBR,AVG_VLL,AVG_VLN FROM ${QUESTDB_TBL} 
        ...
        break;
      case "current":
        sql = `SELECT TS,IR,IY,IB,AVG_I FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limitStmt}
        `;
      ...
      case "pf":
        ...
        sql = `SELECT TS,R_PF,Y_PF,B_PF,AVG_PF FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "frequency":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT TS,FREQUENCY FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "power":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT TS, TOTAL_KW, TOTAL_KVA, TOTAL_KVAR FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "energy":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT TS,R_KVA,Y_KVA,B_KVA,TOTAL_KVA,R_KVAR,Y_KVAR,B_KVAR,TOTAL_KVAR, FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "thd":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT R_THD_V,Y_THD_V,B_THD_V,R_THD_I,Y_THD_I,B_THD_I,KWH,KVAH,KVARH FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "max_demand":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT LN_MAX_DEV FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "load":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT LN_VOL_IMB,LL_MAX_DEV,LL_VOL_IMB,I_MAX_DEV,I_CURR_IMB,F_DEV,F_DEV_PCT,PF_MAX_DEV,PF_IMB FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "efficiency":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT KW_IMB_PCT,KVA_IMB_PCT,DELTA_KWH,DELTA_KVAH,ENG_CHA,KVAR_IMB_PCT FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      case "energy_loss":
        limit = parseInt(query.limit) || 100;
        sql = `SELECT TS, KVARH, KWH, KVAH FROM ${QUESTDB_TBL} 
        WHERE DID='${did}' AND TS >= '${ts_from}' AND TS <= '${ts_to}'
        ORDER BY ts ASC 
        LIMIT ${limit}
        `;
      ...
      default:
        limit = parseInt(query.limit) || 100;
        sql = `SELECT * FROM ${QUESTDB_TBL} WHERE DID='${did}' ORDER BY ts DESC LIMIT ${limit}`;
        break;
    }
...
exports.getRtlTelemetry = async (req, res) => {
  try {
    ...
    const sql = `SELECT * FROM ${QUESTDB_TBL} WHERE DID='${did}' ORDER BY ts DESC LIMIT 1`;
    const { data } = await axios.get(QUESTDB_URL, { params: { query: sql } });
...
exports.getRtlTelemetry2 = async (req, res) => {
  try {
    ...
    const sql = `SELECT * FROM ${QUESTDB_TBL} WHERE DID='${did}' ORDER BY ts DESC LIMIT 2`;
