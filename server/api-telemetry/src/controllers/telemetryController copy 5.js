...
exports.getTelemetry = async (req, res) => {
  try {
    ...
    // Cursor-based pagination for QuestDB (more efficient than offset simulation)
    const cursor = query.cursor; // timestamp cursor for pagination
    let limitStmt = page === 0 ? "" : `LIMIT ${limit}`;
    
    // Build cursor condition for pagination - apply whenever cursor is provided
    let cursorCondition = "";
    if (cursor) {
      cursorCondition = `AND TS < '${cursor}'`; // Use uppercase TS to match database
    }
    ...
    if (cursor) {
      ...
      console.log("Cursor provided:", cursor);
      console.log("Cursor condition:", cursorCondition);
      console.log("==================");
    }
    
    let sql = `SELECT * FROM telemetry_data3 WHERE DID='${did}' ${cursorCondition} ORDER BY TS DESC ${limitStmt}`;
      ...
      case "voltage":
        if (parsedTimeFrame) {
          sql = `SELECT TS,
                 ...
                 WHERE DID='${did}' ${ts_from ? `AND TS >= '${ts_from}'` : ''} ${ts_to ? `AND TS <= '${ts_to}'` : ''} ${cursorCondition}
    ...
    // Get the last timestamp for cursor-based pagination
    let nextCursor = null;
    if (rows.length > 0 && rows.length === limit) {
      // Only provide cursor if we have more data (full page returned)
      // Use TS (uppercase) as that's the actual field name in the database
      nextCursor = rows[rows.length - 1].TS || rows[rows.length - 1].ts;
      
      console.log("=== NEXT CURSOR DEBUG ===");
      console.log("Rows returned:", rows.length);
      console.log("Limit:", limit);
      console.log("Last row TS:", rows[rows.length - 1].TS);
      console.log("Last row ts:", rows[rows.length - 1].ts);
      console.log("Next cursor:", nextCursor);
      console.log("========================");
    }

    // Simplified pagination info for cursor-based pagination
    let totalCount = 0;
    let totalPages = 0;
    
    if (page > 0) {
      // For cursor-based pagination, total count is less critical
      // We can provide it only if explicitly needed
    ...
    if (page > 0) {
      response.pagination = {
        ...
        limit: limit,
        hasNextPage: nextCursor !== null, // More accurate for cursor pagination
        hasPrevPage: page > 1,
        nextCursor: nextCursor // Include cursor for next page
      };
    }
